...
commands = ['standup','start','cancel','next','skip','table','left','ignore','heed','ignoring','help', 'done']
...
def start():
    ...
    post_message('Let\'s get started! What are you working on today? What, if any, are your blockers?')
    ...
    for user in users:
        userString = userString + '@%s, ' % user

    userString = userString + 'please input your status! Type !done when you are finished'

    post_message(userString)
...
def finishUser(message_user):
    global users
    global current_user

    users.remove(message_user)
...
def help(topic=''):
    if topic == '':
        post_message('My commands are !standup, !start, !cancel, !skip, !table, !left, !ignore, !heed, and !ignoring, !done.\nAsk me "!help <command> to learn what they do.')
        return
